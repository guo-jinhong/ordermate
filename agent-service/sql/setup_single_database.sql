...
-- 数据库: ecommerce_db
-- 用途:
--   1. 确保 Spring Boot 业务库 ecommerce_db 存在
--   2. 在同一个库内创建/更新 Agent 知识库表 knowledge
--   3. 创建只读 Agent 数据库账号 agent_user
--
-- 说明:
...
CREATE USER IF NOT EXISTS 'agent_user'@'localhost' IDENTIFIED BY 'agent_readonly_123456';
CREATE USER IF NOT EXISTS 'agent_user'@'%' IDENTIFIED BY 'agent_readonly_123456';

GRANT SELECT ON ecommerce_db.knowledge TO 'agent_user'@'localhost';
GRANT SELECT ON ecommerce_db.knowledge TO 'agent_user'@'%';

FLUSH PRIVILEGES;

SELECT CONCAT('单库模式初始化完成，knowledge 条目数: ', COUNT(*)) AS message FROM knowledge;
